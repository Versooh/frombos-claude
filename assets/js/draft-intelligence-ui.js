...
function futureCost(name){if(!draft().fearless)return 'Fearless OFF';return pool().has(name)?'Consome opção do pool nos próximos jogos':'Custo futuro baixo no pool local';}
function panelHTML(){
  ...
  return `<section class="draft-intelligence card" id="draftIntelligence"><div class="di-head"><div><span class="eyebrow">FROMBOS_STRUCTURAL</span><h3>Draft Intelligence</h3><p class="muted">Leitura heurística do draft. Não é win rate nem dado observado.</p></div><div class="di-actions"><span class="badge gold">${next?`${next[0].toUpperCase()} · ${next[1].toUpperCase()}`:'DRAFT COMPLETO'}</span>${own.length?'<button class="btn info" id="diToTactical">Criar plano tático</button>':''}</div></div>
