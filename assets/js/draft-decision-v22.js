// FROMBOS V22.3 — Draft Decision Layer.
// Reads the active draft DOM and FROMBOS structural heuristics only. No state mutation.
...
function picks(side){return [...document.querySelectorAll(`.draft-team-panel.${side} .dr-slot.pick.filled .dr-slot-copy b`)].map(x=>x.textContent.trim()).filter(Boolean);}
function stage(){const text=document.querySelector('.draft-stage-head h2')?.textContent?.trim()||'';return {text,pick:text.includes('ESCOLHA'),side:text.includes('AZUL')?'blue':text.includes('VERMELHO')?'red':null};}
...
function html(){
  ...
  return `<section class="v22-draft-decision v22-reveal">
    ...
    <footer><b>Como usar:</b> clique em um candidato para localizá-lo na grade. A escolha continua manual e o motor de Draft existente permanece responsável por confirmar o pick.</footer>
  </section>`;
}

function bind(root){
  root.querySelectorAll('[data-v22-draft-candidate]').forEach(btn=>btn.addEventListener('click',()=>{
    const target=[...document.querySelectorAll('#drChampGrid [data-champ]')].find(x=>x.dataset.champ===btn.dataset.v22DraftCandidate);
