...
export function draftRoomHTML(){
  ...
  return `<div class="draft-pro">
    ...
    <div class="draft-arena">
      <section class="draft-team-panel blue"><div class="draft-team-head"><div><span class="eyebrow">BLUE SIDE</span><h2>${esc(store.state.team.name)}</h2></div><span class="side-dot"></span></div><div class="draft-section-label">BANS</div><div class="ban-grid">${slotHTML('blue','ban',5)}</div><div class="draft-section-label">PICKS</div><div class="pick-stack">${slotHTML('blue','pick',5)}</div></section>
      ...
      <section class="draft-team-panel red"><div class="draft-team-head"><div><span class="eyebrow">RED SIDE</span><h2>${esc(store.state.team.opponent)}</h2></div><span class="side-dot"></span></div><div class="draft-section-label">BANS</div><div class="ban-grid">${slotHTML('red','ban',5)}</div><div class="draft-section-label">PICKS</div><div class="pick-stack">${slotHTML('red','pick',5)}</div></section>
    </div>
    <section class="champion-select card">
      <div class="champion-select-head"><div><span class="eyebrow">CHAMPION SELECT</span><h3>${step?'Escolha o próximo campeão':'Draft finalizado'}</h3><p class="muted">Pesquisa instantânea com retratos oficiais de Wild Rift e estados de champion pool/Fearless.</p></div><input class="input" id="drSearch" placeholder="Buscar campeão..." ${!step?'disabled':''}></div>
...
export function bindDraftRoom(rerender){
  ...
  function renderChampions(){const q=($('#drSearch')?.value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');const blocked=previousFearless();const used=new Set(actions().map(x=>x.champ));const next=nextStep();const step=next==null?null:DRAFT_ORDER[next];const pool=new Set(Object.values(store.state.team.players).flatMap(p=>p.pool||[]));const normalize=x=>x.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');$('#drChampGrid').innerHTML=CHAMPIONS.filter(c=>normalize(c).includes(q)).map(c=>{const fearless=blocked.has(c),already=used.has(c),disabled=!step||fearless||already;return `<button class="draft-champ-card ${disabled?'disabled':''} ${pool.has(c)?'in-pool':''}" data-champ="${esc(c)}" ${disabled?'disabled':''}><div class="draft-champ-art">${portraitHTML(c,'draft-champ-portrait')}<span class="draft-champ-state">${fearless?'USED':pool.has(c)?'POOL':'WR'}</span></div><b>${esc(c)}</b>${fearless?'<small>FEARLESS · USED</small>':pool.has(c)?'<small>POOL DO TIME</small>':'<small>WILD RIFT</small>'}</button>`}).join('');document.querySelectorAll('[data-champ]').forEach(b=>b.onclick=()=>pick(b.dataset.champ));}
  ...
  document.querySelectorAll('[data-branch]').forEach(b=>b.onclick=()=>{const branch=d().branches.find(x=>x.id===b.dataset.branch);if(!branch)return;store.update(s=>{s.draft.game=branch.game;s.draft.actions=branch.actions.map(x=>({...x}));s.draft.games[s.draft.game]={actions:s.draft.actions.map(x=>({...x}))};});rerender();});renderChampions();
}
