...
function ensureScrim(){
  ...
  if(!scrim){
    ...
    scrim.type='button';scrim.className='mobile-scrim-v20';scrim.setAttribute('aria-label','Fechar menu');
    scrim.onclick=closeNav;document.body.appendChild(scrim);
  }
  return scrim;
}
function setState(open){
  ...
  sidebar.classList.toggle('open',active);document.body.classList.toggle('mobile-nav-open-v20',active);scrim.classList.toggle('active',active);
  sidebar.setAttribute('aria-hidden',active?'false':mobile()?'true':'false');
...
function bindShell(){
  const sidebar=q('#sidebar'),toggle=q('#menuToggle');if(!sidebar||!toggle)return;
  ensureScrim();
  ...
  qa('[data-route]',sidebar).forEach(item=>{
    if(item.dataset.v20MobileBound==='1')return;item.dataset.v20MobileBound='1';item.addEventListener('click',closeNav);
  });
...
window.addEventListener('hashchange',()=>{closeNav();schedule();});window.addEventListener('resize',schedule);window.addEventListener('keydown',onKey);window.addEventListener('load',schedule);schedule();
