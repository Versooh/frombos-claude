...
import { mergeWorkspace } from './workspace.js';
...
export async function listTeams(organizationId = getActiveOrgId()) {
  ...
  const { data, error } = await supabase
    .from('teams')
    .select('id, organization_id, name, slug, game, created_at')
...
export async function createTeam(name, slug, game = 'wild_rift', organizationId = getActiveOrgId()) {
  ...
  const { data, error } = await supabase
    .from('teams')
    .insert({ organization_id: organizationId, name: name.trim(), slug: slug.trim().toLowerCase(), game })
    .select('id, organization_id, name, slug, game, created_at')
    .single();
  if (error) throw error;
...
export async function listSeasons(teamId = getActiveTeamId()) {
  ...
  const { data, error } = await supabase
    .from('team_seasons')
    .select('id, team_id, label, starts_on, ends_on, active, created_at')
    .eq('team_id', teamId)
...
export async function createSeason(teamId, label, startsOn = null, endsOn = null, active = true) {
  ...
  const { data, error } = await supabase
    .from('team_seasons')
    .insert({ team_id: teamId, label: label.trim(), starts_on: startsOn || null, ends_on: endsOn || null, active })
    .select('id, team_id, label, starts_on, ends_on, active, created_at')
    .single();
  if (error) throw error;
...
export async function listPlayers(seasonId = getActiveSeasonId()) {
  ...
  const { data, error } = await supabase
    .from('players')
    .select('id, team_season_id, display_name, primary_role, secondary_roles, active, created_at')
...
export async function listChampionPools(playerIds = []) {
  ...
  await requireUser();
  const { data, error } = await supabase
    .from('player_champion_pool')
    .select('player_id, champion_id, role, tier, coach_note, last_validated_at, updated_by, updated_at')
...
export function hydrateCoreFromCloud(snapshot) {
  ...
  for (const pool of snapshot.pools || []) {
    (poolsByPlayer[pool.player_id] ||= []).push(pool.champion_id);
  }
  coreStore.update(core => {
    core.team.id = snapshot.team.id;
    core.team.name = snapshot.team.name;
    core.team.opponent = core.team.opponent || '';
    ...
    core.meta.cloud = {
      ...
      teamSeasonId: snapshot.season?.id || null,
